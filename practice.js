...
movements.forEach(function(movement, i,arr){ //  continue and break doesnt work here 
...
const movementDesc= movements.map(function(mov,i,arr){
 return  `Movement ${i+1}: You Deposited ${mov>0?'deposited':'Withdrew'} ${Math.abs(mov)} `
...
//Pipline
const TotalDepositsInd= movements.filter((mov)=>mov>0).map((mov)=>mov*eurtoUsd).reduce((acc,cur)=>acc+cur,0);
...
const calcAverageHumanAg=function(movement){
 const age= movement.map((age)=>age<=2?2*age:16+age*4).
 filter((age)=>age>=18).
...
const  ovrt = accounts.map((acc=>acc.movements)).flat().reduce((acc,mov)=>acc+mov,0);
console.log(ovrt );
// Flat Map  is used to do Flat and Map in array at Same time
const flatMp= accounts.flatMap((acc)=>acc.movements).reduce((acc,mov)=>acc+mov,0);
console.log(flatMp);
...
//  Lectute 166
const bankdepo =  accounts.flatMap(acc=>acc.movements)
.filter(acc=>acc>0)
...
const numdepored=accounts.flatMap(acc=>acc.movements)
.reduce((acc,cur)=>(cur>=1000?++acc:acc),0);
console.log(numdepored);

// 3.Advance reduce  creation of new object

const {deposis,withdrawals} =accounts.flatMap(acc=>acc.movements)
.reduce((acc,cur)=>{
...
//2.Find the Sarh dog
dogs.forEach((dog)=>{
 const own=dog.owners.flatMap((ownr)=>{ownr=='Sarah'?(dog.curFood>dog.RecFood?console.log('Sarah Dog Eating high'):console.log('Sarah dog Eating low')):console.log(ownr)});
//  console.log(own);
})
const sarahdog= dogs.find((dog)=>dog.owners.includes('Sarah'));
console.log(sarahdog);
console.log(` Sarah dog is eating ${sarahdog.curFood>sarahdog.RecFood?'much':'Low'}`);
// console.log();
